...
class FBLogin extends Component {
  ...
  componentDidMount() {
    LoginManager.setLoginBehavior(this.props.loginBehavior).then(behaviour => {
     // console.log(`FbLogin: using ${behaviour.name} behaviour`, behaviour);
    });
    LoginManager.getCredentials((err, data) => {
      if (
        data &&
  ...
  logout() {
    LoginManager.logout((err, data) => this._handleEvent(err, data));
  }
